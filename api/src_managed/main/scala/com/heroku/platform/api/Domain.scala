...
case class DomainIdentity(id: Option[String], hostname: Option[String])

case object DomainIdentity {
  def byId(id: String) = DomainIdentity(Some(id), None)
  def byHostname(hostname: String) = DomainIdentity(None, Some(hostname))
}

trait DomainRequestJson {
  implicit def ToJsonCreateDomainBody: ToJson[models.CreateDomainBody]
  implicit def ToJsonDomainIdentity: ToJson[DomainIdentity]
}
