...
case class PlanIdentity(id: Option[String], name: Option[String])

case object PlanIdentity {
  def byId(id: String) = PlanIdentity(Some(id), None)
  def byName(name: String) = PlanIdentity(None, Some(name))
}

trait PlanRequestJson {
  implicit def ToJsonPlanPrice: ToJson[models.PlanPrice]
  implicit def ToJsonPlanIdentity: ToJson[PlanIdentity]
}
