...
case class SslEndpointIdentity(id: Option[String], name: Option[String])

case object SslEndpointIdentity {
  def byId(id: String) = SslEndpointIdentity(Some(id), None)
  def byName(name: String) = SslEndpointIdentity(None, Some(name))
}
...
trait SslEndpointRequestJson {
  ...
  implicit def ToJsonSslEndpointIdentity: ToJson[SslEndpointIdentity]
}
