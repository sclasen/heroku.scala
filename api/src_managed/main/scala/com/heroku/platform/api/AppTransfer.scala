...
object AppTransfer {
  ...
  object models {
    case class CreateAppTransferBody(app: HerokuAppIdentity, recipient: AccountIdentity)
  ...
  case class Create(app: HerokuAppIdentity, recipient: AccountIdentity) extends RequestWithBody[models.CreateAppTransferBody, AppTransfer] {
    val expect: Set[Int] = expect201
    val endpoint: String = "/account/app-transfers"
    val method: String = POST
    val body: models.CreateAppTransferBody = models.CreateAppTransferBody(app, recipient)
  }
...
case class AppTransferIdentity(id: Option[String])

case object AppTransferIdentity {
  def byId(id: String) = AppTransferIdentity(Some(id))
}
...
trait AppTransferRequestJson {
  ...
  implicit def ToJsonAppTransferIdentity: ToJson[AppTransferIdentity]
}
