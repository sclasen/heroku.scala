...
case class FormationIdentity(id: Option[String], `type`: Option[String])

case object FormationIdentity {
  def byId(id: String) = FormationIdentity(Some(id), None)
  def byType(`type`: String) = FormationIdentity(None, Some(`type`))
}

trait FormationRequestJson {
  implicit def ToJsonUpdateFormationBody: ToJson[models.UpdateFormationBody]
  implicit def ToJsonFormationIdentity: ToJson[FormationIdentity]
}
