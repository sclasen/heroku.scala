...
object Collaborator {
  ...
  object models {
    case class CreateCollaboratorBody(silent: Option[Boolean] = None, user: AccountIdentity)
    case class CollaboratorUser(email: String, id: String)
  }
  case class Create(app_id_or_name: String, silent: Option[Boolean] = None, user: AccountIdentity) extends RequestWithBody[models.CreateCollaboratorBody, Collaborator] {
...
case class CollaboratorIdentity(email: Option[String], id: Option[String])

case object CollaboratorIdentity {
...
trait CollaboratorRequestJson {
  ...
  implicit def ToJsonCollaboratorIdentity: ToJson[CollaboratorIdentity]
}
