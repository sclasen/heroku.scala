...
object Key {
  ...
  case class Create(publicKey: String) extends RequestWithBody[CreateKeyBody, Key] {
  ...
  case class Info(keyId: String) extends Request[Key] {
    val endpoint: String = s"/account/keys/$keyId"
    val method: String = GET
    val expect = expect200
  }

  case class Delete(keyId: String) extends Request[Key] {
    val endpoint: String = s"/account/keys/$keyId"
